						...
						if (tp.getRepetitiveFloorNos().contains(floor.getNumber()))
							for (Floor allFloors : block.getBuilding().getFloors())
								if (allFloors.getNumber().equals(tp.getModelFloorNo()))
									if (!allFloors.getBalconies().isEmpty()) {
										System.out.println("TYPICAL_BALCONY__"+floor.getNumber()+", "+allFloors.getBalconies().size());
										floor.setBalconies(allFloors.getBalconies());
										continue outside;
									}
